...
class RMTForReasoning(PreTrainedModel):
    ...
    @classmethod
    def from_pretrained(
        cls, pretrained_model_name_or_path, config=None, *args, **kwargs
    ):
...
class RecurrentWrapperNoSegmentation(RecurrentWrapper):
    def forward(
        self, segments, labels, output_attentions=None, output_hidden_states=None
    ):
        memory_state = None
    ...
    def process_outputs(self, cell_outputs, segments, **kwargs):
        ...
        out["loss"] = (
            sum([proxy_out[f"loss_{seg_num}"] for seg_num in range(num_segments)])
            / num_segments
        )
...
class RecurrentWrapperNoSegmentationGenerate(RecurrentWrapperNoSegmentation):
    ...
    def generate(self, segments, **kwargs):
        ...
        for seg_num in range(len(segments), self.rmt_config.get("max_n_segments", 32)):
            output_ids, memory_state = self.generate_segment(
                memory_state=memory_state, **kwargs
            )
    ...
    def generate_segment(self, memory_state, **kwargs):
        ...
        _, memory_state = self.memory_cell(
            input_ids=fwd_inputs, memory_state=memory_state
        )
    ...
    def all_done(self, generated_segments):
        ...
        have_eos = [
            any([eos in seg[i] for seg in generated_segments]) for i in range(bs)
        ]
